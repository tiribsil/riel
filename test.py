...
class TestExamesEmColeta(unittest.TestCase):
    def setUp(self):
        self.lista = ExamesEmColeta()
        self.exames = [
            ExameLaboratorial("Hemograma", "Análise de células sanguíneas"),
            ExameLaboratorial("Colesterol Total", "Medição do colesterol no sangue"),
            ExameLaboratorial("Glicemia", "Medição da glicose no sangue"),
    ...
    def test_adicionar_registro_lista_vazia(self):
        self.assertTrue(self.lista.inserir(self.registros[1]))
        self.assertEqual(str(self.lista), str(self.registros[1]))
        self.assertTrue(self.lista.inserir(self.registros[4]))
        self.assertEqual(str(self.lista), str(self.registros[1]) + '\n' + str(self.registros[4]))

    def test_remover_registro(self):
        cpf = self.registros[4].get_paciente().get_cpf()    
        self.lista.inserir(self.registros[4])
        self.assertEqual(self.lista.remover(cpf), self.registros[4])
        self.assertEqual(len(self.lista), 0)

    def test_remover_de_lista_vazia(self):
        cpf = self.registros[8].get_paciente().get_cpf()
